...
A build-time pipeline that, alongside your normal build output, produces a structured knowledge graph — persisted, queryable, and cumulative across time. Each build adds nodes and edges; nothing is thrown away.

The graph knows:

- **what changed** — files, functions, modules, with the diff tied to the commit and the rationale from the PR/issue
